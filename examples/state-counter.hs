...
stateExample = do
  ...
  t <- runSigmacord $ def { SigmacordToken = tok
                        , SigmacordOnStart = writeChan printQueue "starting ping loop"
                        , SigmacordOnEvent = eventHandler state printQueue
                        , SigmacordOnEnd = do killThread threadId
                                            --
                                            s <- readMVar state
...
eventHandler :: MVar State -> Chan T.Text -> Event -> SigmacordHandler ()
