...
main = do
  ...
  botTerminationError <- runSigmacord $ def
    { SigmacordToken = tok
    , SigmacordOnEvent = onSigmacordEvent testGuildId
    -- If you are using application commands, you might not need
    -- message contents at all
    , SigmacordGatewayIntent = def { gatewayIntentMessageContent = False }
    }
...
data SlashCommand = SlashCommand
  ...
  , handler :: Interaction -> Maybe OptionsData -> SigmacordHandler ()
  }
...
onSigmacordEvent :: GuildId -> Event -> SigmacordHandler ()
onSigmacordEvent testGuildId = \case
  Ready _ _ _ _ _ _ (PartialApplication appId _) -> onReady appId testGuildId
  InteractionCreate intr                         -> onInteractionCreate intr
  _                                              -> pure ()

onReady :: ApplicationId -> GuildId -> SigmacordHandler ()
...
onInteractionCreate :: Interaction -> SigmacordHandler ()
